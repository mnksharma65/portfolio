...
for ($i = 0; $i < $length; $i++) {
   if ($ca[$i] != '')
   {
  $query = "insert into state (name, countryId) values('".$ca[$i]."', '".$cid."' )";
  mysql_query($query) or die('Invalid query: ' . mysql_error());
   }
       
}
   echo "<script type='text/javascript'>window.location.href='manage_state.php?status=Success'</script>";
 
}
elseif ($_GET['name']=='deleteCountry')
{
    $id = $_GET['data'];
    $version = mysql_query("Delete from country WHERE id= $id");
   ...
   /* $response['responseCode'] = "00";
    ...
    echo "<script type='text/javascript'>window.location.href='manage_country.php?data=Deleted'</script>";
