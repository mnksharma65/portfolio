                        ...
                        <div class="row">
                          <div class="large-12 medium-12 columns">
                            <table class="responsive">
                               <tr><th>ID</th><th>Username</th><th>Email</th><th>Password</th><th>Salt</th><th>&nbsp;</th></tr>
                                <?php
                                    $i = $start+1;
                                    while($project = mysql_fetch_array($query_project))
                                        {
                                            $project = mysql_query("select * from members where id = '".$project['id']."' ");
                                            $result_project = mysql_fetch_array($project);
                                            
                                            echo '<tr><td>'.$i.'</td><td>'.$result_project['username'].'</td>';
                                            echo '<td>'.$result_project['email'].'</td>';
                                            echo '<td>'.$result_project['password'].'</td>';
                                            echo '<td>'.$result_project['salt'].'</td>';
                                            $response['id'] = $result_project['id'];
                                            $response['username'] = $result_project['username'];
                                            $response = base64_encode(json_encode($response));
                                            echo '<td><a href="edit_member.php?data='.$response.'"><img src="images/update.png"  alt="Update Survey" title="Update Survey"/></a>
                                                
                                                <a href="service_member.php?name=deleteMember&&data='.$response.'" onclick="confirm()"><img src="images/delete.png" onclick="myFunction()" "alt="Delete Survey" title="Delete Survey"/></a>
                                                </td></tr>';
                                            $response = "";
                                            $i++;
                                    }
                              ...
                              <?php
                                echo '<a href="addMember.php" class="tiny button secondary" style="background-color: #81BEF7">Add Members</a>';
                                if($nume > $limit ){
